...
export function getHost(urlOrg: string) {
  ...
  try {
    ...
    console.log('common.ts error', urlOrg);
    process.exit(0);
  }
}
...
export function urlPageNum(urls: string, currentPage: number): string {
  ...
  // если идет неавторизованный запрос - то обязательно ставим галку "сначала из моего города" - иначе выдастся только первая страница
  if (!searchParams.has('localPriority')) {
    searchParams.append('localPriority', '1');
  }
